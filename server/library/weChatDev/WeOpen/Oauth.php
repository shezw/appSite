...
class Oauth extends BasicWeChat
{
    ...
    public function getOauthRedirect(string $redirect_url, $state = '', $scope = 'snsapi_login'): string
    {
        ...
        return "https://open.weixin.qq.com/connect/qrconnect?appid={$appid}&redirect_uri={$redirect_uri}&response_type=code&scope={$scope}&state={$state}&connect_redirect=1#wechat_redirect";
    }
    ...
    {
        $appid = $this->config->get('openkey');
        $appsecret = $this->config->get('opensecret');
